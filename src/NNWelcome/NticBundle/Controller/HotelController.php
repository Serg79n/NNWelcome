...
use NNWelcome\NticBundle\Entity\Hotel;
use NNWelcome\NticBundle\Entity\HotelImage;
use NNWelcome\NticBundle\Entity\HotelFile;
use NNWelcome\NticBundle\Form\HotelType;
use NNWelcome\NticBundle\Form\HotelFilterType;
...
class HotelController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        if ($page) {
            $request->getSession()->set('admin_hotel_page', $page);
        } elseif ($request->getSession()->has('admin_hotel_page')) {
            $page = $request->getSession()->get('admin_hotel_page');
        } else {
            $page = 1;
        }
        ...
        $filterForm = $this->createForm(new HotelFilterType(), $this->getFilterModel($request));
    ...
    public function filterAction(Request $request) {
        $form = $this->createForm(new HotelFilterType());
        $form->bind($request);

        $this->setFilter($request, $form);
        $request->getSession()->set('admin_hotel_page', 1);

        return $this->redirect($this->generateUrl('admin_hotel'));
    }

    public function resetFilterAction(Request $request) {
        $request->getSession()->remove('admin_hotel_filter');
        $request->getSession()->set('admin_hotel_page', 1);
    ...
    private function BuildQuery($request) {
        ...
        $this->query = $em->getRepository('NticBundle:Hotel')
                ->retriveHotel($request);
    ...
    private function setFilter($request, $form) {
        ...
        foreach ($form as $one) {
            if ($one->getData() !== null) {
                $type = $this->getDbType('NNWelcome\NticBundle\Entity\Hotel', $one->getName());
                if ($type == 'entity')
        ...
        $request->getSession()->set('admin_hotel_filter', $filter);
    }

    private function getFilter($request) {
        if ($request->getSession()->has('admin_hotel_filter')) {
            foreach ($request->getSession()->get('admin_hotel_filter') as $i => $one) {
    ...
    private function getFilterModel($request) {
        ...
        if ($request->getSession()->has('admin_hotel_filter')) {
            foreach ($request->getSession()->get('admin_hotel_filter') as $i => $one) {
                if ($one['type'] == 'entity') {
                    $metadata = $this->getMetadatas('NNWelcome\NticBundle\Entity\Hotel');
    ...
    public function newAction(Request $request) {
        ...
        $page = 1;
        if ($request->getSession()->has('admin_hotel_page')) {
            $page = $request->getSession()->get('admin_hotel_page');
        }
        return array(
            'page' => $page,
    ...
    /**
     * @Template("NticBundle:Hotel:new.html.twig")
     */
    public function createAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            return $this->redirect($this->generateUrl('admin_hotel_edit', array('id' => $entity->getId())));
        }
    ...
    public function editAction($id, Request $request) {
        $em = $this->getDoctrine()->getManager();
        $entity = $em->getRepository('NticBundle:Hotel')->find($id);
    ...
    /**
     * @Template("NticBundle:Hotel:edit.html.twig")
     */
    ...
    public function changeActiveAction(Request $request) {
        ...
        return $this->render(
                        'NticBundle:Hotel:_ajaxResponse.html.twig', array(
                    'text' => $active
                        )
        );
    }
