...
use NNWelcome\LocationBundle\Entity\City;
use NNWelcome\LocationBundle\Form\CityType;
use NNWelcome\LocationBundle\Form\CityFilterType;
...
class CityController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        if ($page) {
            $request->getSession()->set('admin_city_page', $page);
        } elseif ($request->getSession()->has('admin_city_page')) {
            $page = $request->getSession()->get('admin_city_page');
        } else {
            $page = 1;
        }
        ...
        $filterForm = $this->createForm(new CityFilterType(), $this->getFilterModel($request));
    ...
    private function BuildQuery($request) {
        ...
        $this->query = $em->getRepository('LocationBundle:City')
                ->retriveCity($request);
    ...
    private function setFilter($request, $form) {
        ...
        foreach ($form as $one) {
            if ($one->getData() !== null) {
                $type = $this->getDbType('NNWelcome\LocationBundle\Entity\City', $one->getName());
                if ($type == 'entity')
        ...
        $request->getSession()->set('admin_city_filter', $filter);
    }

    private function getFilter($request) {
        if ($request->getSession()->has('admin_city_filter')) {
            foreach ($request->getSession()->get('admin_city_filter') as $i => $one) {
    ...
    private function getFilterModel($request) {
        ...
        if ($request->getSession()->has('admin_city_filter')) {
            foreach ($request->getSession()->get('admin_city_filter') as $i => $one) {
                if ($one['type'] == 'entity') {
                    $metadata = $this->getMetadatas('NNWelcome\LocationBundle\Entity\City');
    ...
    public function newAction(Request $request) {
        ...
        $page = 1;
        if ($request->getSession()->has('admin_city_page')) {
            $page = $request->getSession()->get('admin_city_page');
        }
        return array(
            'page' => $page,
    ...
    /**
     * @Template("LocationBundle:City:new.html.twig")
     */
    public function createAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            return $this->redirect($this->generateUrl('admin_city_edit', array('id' => $entity->getId())));
        }
    ...
    public function editAction($id, Request $request) {
        $em = $this->getDoctrine()->getManager();
        $entity = $em->getRepository('LocationBundle:City')->find($id);
    ...
    /**
     * @Template("LocationBundle:City:edit.html.twig")
     */
    ...
    public function changeActiveAction(Request $request) {
        ...
        return $this->render(
                        'LocationBundle:City:_ajaxResponse.html.twig', array(
                    'text' => $active
                        )
        );
    }
